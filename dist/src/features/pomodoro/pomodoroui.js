...
export class PomodoroUi {
    ...
    getPomodoroStatusAndCycleText(sessionInfo, appState) {
        ...
        switch (appState) {
            ...
            case MyAppState.POMODORO_PENDING_CONFIRMATION:
                ...
                if (prevPhase === 'WORK')
                    return `Work Finished (${pomodorosDone}/${intervalsTotal})`;
                if (prevPhase === 'SHORT_BREAK')
                    return `Short Break Finished (${pomodorosDone}/${intervalsTotal} work done)`;
    ...
    displayInterface(session, sessionId, sessionInfo, appState) {
