...
export class TimerUi {
    ...
    showConfigurationScreen(session, sessionId, sessionInfo) {
    ...
    showFinishedScreen(session, sessionId, sessionInfo) {
        if (!sessionInfo.isConnected || !sessionInfo.currentSettings) {
        ...
        if (stateText)
            finishedDisplayText += `${stateText}\n`;
        if (mainMessage)
            finishedDisplayText += `${mainMessage}\n`;
        if (voiceCommandHint)
            finishedDisplayText += `\n${voiceCommandHint}`;
        try {
