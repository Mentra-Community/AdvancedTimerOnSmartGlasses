...
const ALWAYS_LISTEN_SDK_KEYS = [ /* ... */];
const FEATURE_TOGGLE_SDK_KEYS = [ /* ... */];
const TIMER_DATA_SDK_KEYS = [ /* ... */];
const STOPWATCH_DATA_SDK_KEYS = [ /* ... */];
const POMODORO_DATA_SDK_KEYS = [ /* ... */];
const SDK_KEY_TO_APP_SETTINGS_KEY_MAP = { /* ... */};
export class AdvancedTimerApp extends AppServer {
    ...
    ownPort;
    ownApiKey;
    settingsRefreshedUnsubscribeMap = new Map();
    initializationTimeouts = new Map();
    transcriptionUnsubscribeMap = new Map();
    constructor(config) {
        super(config);
        ...
        this.commandHandler = new CommandHandler(this.sessionManager, this.uiManager, this.stopwatchLogic, this.timerLogic, this.pomodoroLogic, this.settingsManager);
        console.log('[AdvancedTimerApp] All components initialized.');
    }
    _getEffectiveSettingsForSession(sessionId) {
        const effectiveSettings = { ...fallbackDefaultAppSettings };
        const keys = Object.keys(fallbackDefaultAppSettings);
        for (const key of keys) {
            effectiveSettings[key] = this.settingsManager.getSetting(sessionId, key);
        }
        return effectiveSettings;
    }
    _stopPreviousModeDataListeners(sessionId, previousState) {
    ...
    async _handleAppInitializing(session, sessionId, stage = 1) {
    ...
    async onSession(session, sessionId, userId) {
        ...
        const subscribeTranscriptionIfNeeded = () => {
            ...
            if (sessInfo && sessInfo.currentControlMethod === 'voice') {
                ...
                if (prevUnsub)
                    prevUnsub();
                const unsub = session.events.onTranscription(async (transcriptionData) => {
                    const sessionInfo = this.sessionManager.getSessionInfo(sessionId);
                    if (!sessionInfo || !sessionInfo.isConnected || !sessionInfo.currentSettings || this.sessionManager.getState(sessionId) === MyAppState.APP_INITIALIZING)
                        return;
                    const effectiveControlMethod = sessionInfo.currentControlMethod;
                    if (effectiveControlMethod !== 'voice')
                        return;
        ...
        const unsubscribeSettingsRefreshed = this.settingsManager.onSettingsRefreshed(sessionId, (changedTpaConfigKeys) => {
            ...
            changedTpaConfigKeys.forEach(tpaConfigKey => {
                ...
                let appSettingEquivalentKey = tpaConfigKey;
                let valueToCheckAgainstPrevious = newEffectiveSettings[appSettingEquivalentKey];
                let previousValueInSession = previousAppSettingsInSession[appSettingEquivalentKey];
                if (tpaConfigKey === 'default_timer_duration_minutes') {
                    ...
                    if (previousValueInSession !== valueToCheckAgainstPrevious) {
                        ...
                        this.timerLogic.handleSettingChange(session, sessionId, 'default_timer_duration_seconds', newEffectiveSettings.default_timer_duration_seconds);
                    }
                    ...
                    if (tpaConfigKey === 'startup_mode') {
                        ...
                        const displayMax = valueToCheckAgainstPrevious >= 99 ? "Unlimited" : `${valueToCheckAgainstPrevious} rounds`;
                ...
                else if (!Object.prototype.hasOwnProperty.call(previousAppSettingsInSession, appSettingEquivalentKey) && newEffectiveSettings[appSettingEquivalentKey] !== undefined) {
                    console.log(`[AdvancedTimerApp] New setting '${appSettingEquivalentKey}' (from tpaKey '${tpaConfigKey}') detected with value '${newEffectiveSettings[appSettingEquivalentKey]}'`);
                }
            });
            if (uiRefreshNeeded) {
        ...
        session.events.onDisconnected((disconnectData) => {
    ...
    async handleActivateToggleChange(session, sessionId, featureType, newToggleValue, oldToggleValueInSession) {
        ...
        if (newToggleValue === true) {
            ...
            if (shouldStartFeature) {
                switch (featureType) {
                    case 'timer':
                        if (currentState === MyAppState.TIMER_RUNNING || currentState === MyAppState.TIMER_PAUSED || currentState === MyAppState.TIMER_READY)
                            shouldStartFeature = false;
                        break;
                    case 'stopwatch':
                        if (currentState === MyAppState.STOPWATCH_RUNNING || currentState === MyAppState.STOPWATCH_PAUSED || currentState === MyAppState.STOPWATCH_READY)
                            shouldStartFeature = false;
                        break;
                    case 'pomodoro':
                        if (currentState === MyAppState.POMODORO_IDLE || currentState === MyAppState.POMODORO_WORK_RUNNING || currentState === MyAppState.POMODORO_BREAK_RUNNING || currentState === MyAppState.POMODORO_PAUSED)
                            shouldStartFeature = false;
                        break;
                }
            }
            ...
            if (shouldStopFeature) {
                ...
                switch (featureType) {
                    case 'timer':
                        isFeatureCurrentlyActive = (currentState === MyAppState.TIMER_RUNNING || currentState === MyAppState.TIMER_PAUSED || currentState === MyAppState.TIMER_READY || currentState === MyAppState.CONFIGURING_TIMER);
                        break;
                    case 'stopwatch':
                        isFeatureCurrentlyActive = (currentState === MyAppState.STOPWATCH_RUNNING || currentState === MyAppState.STOPWATCH_PAUSED || currentState === MyAppState.STOPWATCH_STOPPED || currentState === MyAppState.STOPWATCH_READY || currentState === MyAppState.STOPWATCH_AUTOLAP_COMPLETED);
                        break;
                    case 'pomodoro':
                        isFeatureCurrentlyActive = (currentState === MyAppState.POMODORO_IDLE || currentState === MyAppState.POMODORO_WORK_RUNNING || currentState === MyAppState.POMODORO_BREAK_RUNNING || currentState === MyAppState.POMODORO_PAUSED || currentState === MyAppState.POMODORO_PENDING_CONFIRMATION || currentState === MyAppState.POMODORO_CYCLE_ENDED);
                        break;
                }
    ...
    handlePomodoroDurationChange(_appSettingsKey, newDurationSeconds, toastLabelPrefix, session, sessionId) {
    ...
    applyStartupMode(session, sessionId) {
        const sessionInfo = this.sessionManager.getSessionInfo(sessionId);
    ...
    refreshCurrentUI(session, sessionId) {
        const currentState = this.sessionManager.getState(sessionId);
        const sessionInfo = this.sessionManager.getSessionInfo(sessionId);
        ...
        switch (currentState) {
            ...
            case MyAppState.STOPWATCH_AUTOLAP_COMPLETED:
                this.stopwatchUi.displayInterface(session, sessionId, sessionInfo, currentState);
                break;
            ...
            case MyAppState.TIMER_RUNNING:
                this.timerUi.displayInterface(session, sessionId, sessionInfo, currentState);
                break;
            ...
            case MyAppState.POMODORO_CYCLE_ENDED:
                this.pomodoroUi.displayInterface(session, sessionId, sessionInfo, currentState);
                break;
            default:
