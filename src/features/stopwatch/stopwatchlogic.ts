...
export class StopwatchLogic {
    ...
    private _clearStopwatchInterval(sessionId: string): void {
    ...
    private _handleAutoLap(session: AppSession, sessionId: string): void {
        const sessionInfo = this.sessionManager.getSessionInfo(sessionId);
    ...
    public lap(session: AppSession, sessionId: string, isAutoLap: boolean = false): void {
        let sessionInfo = this.sessionManager.getSessionInfo(sessionId);
        const currentState = this.sessionManager.getState(sessionId);
        if (!sessionInfo || !sessionInfo.isConnected || !sessionInfo.currentSettings)  {
    ...
    public pause(session: AppSession, sessionId: string): void {
        ...
        if (currentState === MyAppState.STOPWATCH_RUNNING) {
            ...
            if (updatedSessionInfo) console.log(`[StopwatchLogic] Session ${sessionId}: Stopwatch paused at ${formatTime(updatedSessionInfo.elapsedSeconds)}.`);
        } else {
    ...
    public async stopStopwatch(session: AppSession, sessionId: string, toastMessage?: string): Promise<void> {
    ...
    public async requestModeSelection(session: AppSession, sessionId: string): Promise<void> {
        const currentState = this.sessionManager.getState(sessionId);
        if (currentState === MyAppState.STOPWATCH_RUNNING || 
     ...
     */
    public handleSettingChange(
        session: AppSession, 
        sessionId: string, 
        changedSettingKey: keyof AppSettings, 
        newValue: any
    ): void {
        const sessionInfo = this.sessionManager.getSessionInfo(sessionId);
        const currentState = this.sessionManager.getState(sessionId);
            ...
            if (changedSettingKey === 'stopwatch_auto_lap_enabled' || 
                ...
                if (currentState === MyAppState.STOPWATCH_READY || currentState === MyAppState.STOPWATCH_PAUSED || currentState === MyAppState.STOPWATCH_AUTOLAP_COMPLETED) {
                    ...
                    if (updatedSessionInfo) {
                        this.stopwatchUi.displayInterface(session, sessionId, updatedSessionInfo, currentState as any);
